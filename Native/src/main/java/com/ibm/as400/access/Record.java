...
//@B5D  transient private Vector vetoListeners_ = new Vector();
     ...
     * @throws UnsupportedEncodingException If an error occurs when converting
    ...
    public Record(RecordFormat recordFormat, byte[] contents, int offset)
        ...
        for (int i = 0; i < fields_.length; ++i) {
            ...
            if (fd instanceof VariableLengthFieldDescription) {
                if (((VariableLengthFieldDescription) fd).isVariableLength()) { // Add two bytes for the length of the field
                    fieldOffset += 2;
                    recordLength_ += 2;
                }
            }
        ...
        if (!hasDependentFields_) { // Go ahead and copy the contents to as400Data_
            try {
                System.arraycopy(contents, offset, as400Data_, 0, as400Data_.length);
            } catch (ArrayIndexOutOfBoundsException e) { // Need to reset isConverted... state
...
//@B5D    currentVetoListeners_ = (Vector)vetoListeners_.clone();
     ...
     * @throws CharConversionException      If an error occurs when converting
     *                                      the contents of a field to IBM i data.
     * @throws UnsupportedEncodingException If an error occurs when converting
     *                                      the contents of a field to IBM i data.
     * @see Record#Record(RecordFormat)
    ...
    public byte[] getContents()
        ...
        if (hasDependentFields_) { // If the record has dependent fields, we do the conversion now and return the contents.
            ...
            for (int i = 0; i < fields_.length; ++i) { // Convert each field to server data
                f = fieldDescriptions_[i];
                // Check for possible variable length field
                if (f instanceof VariableLengthFieldDescription) {
                    if (((VariableLengthFieldDescription) f).isVariableLength()) { // Set the two byte length portion of the contents
                        if (fields_[i] == null) {
                ...
                // level access.
                dType = f.getDataType();
                if (fields_[i] != null) { // Field has a value; convert it to server data
                    offset += dType.toBytes(fields_[i], toBytes, offset);
                } else { // Field is null; use the default value for the server data to serve as a place holder for
                    // the field.
     ...
     * @throws IOException If an I/O error occurs while communicating with the system.
     * @see Record#Record(RecordFormat)
     * @see Record#setRecordFormat
     **/
    ...
    public Object getField(int index)
        ...
        if (isConvertedToJava_[index]) { // Field already converted; just return it
        ...
        // Check for possible variable length field
        if (f instanceof VariableLengthFieldDescription) {
            if (((VariableLengthFieldDescription) f).isVariableLength()) { // Get the number of bytes returned for the field
                ...
                if ((f instanceof HexFieldDescription)) { // Field is a hex field, no conversion is done on the data
                    byte[] b = new byte[variableFieldLength];
                    System.arraycopy(as400Data_, offset, b, 0, variableFieldLength);
                    fields_[index] = b;
                } else { // Field requires conversion based on ccsid
    ...
    public byte[] getFieldAsBytes(int index) {
        ...
        if (hasDependentFields_) // The record has dependent fields, so fieldOffsets_ is unreliable.
        {
            ...
            for (int i = 0; i <= index; ++i) {
                ...
                if (offsetDependField != -1) { // Set offset to value specified in the field this field depends on
        ...
        else  // no dependent fields, so fieldOffsets_ is reliable
        {
            ...
            // Check for possible variable length field.
            if (f instanceof VariableLengthFieldDescription &&
                    ((VariableLengthFieldDescription) f).isVariableLength()) { // Get the number of bytes returned for the field.
                // Whether the field is variable length or not we still write out the maximum number
...
//@RBA
    ...
    private void initializeRecord(RecordFormat recordFormat) {
        ...
        for (int i = 0; i < fields_.length; ++i) {
            ...
            // Check for possible variable length field
            if (fd instanceof VariableLengthFieldDescription) {
                if (((VariableLengthFieldDescription) fd).isVariableLength()) { // Set the two byte length portion of the contents
                    if (obj == null) {
                        variableFieldLength = 0;
                    } else {
...
//        if (dt instanceof AS400Text)
     ...
     * @throws ClassNotFoundException If the class being deserialized is not found.
...
//@B5D    currentVetoListeners_ = (Vector)vetoListeners_.clone();
    ...
    public void setContents(byte[] contents, int offset)
        ...
        if (hasDependentFields_) { // Record has dependent fields; convert now.
            ...
            for (int i = 0; i < numFields; ++i) {
                ...
                if (lengthDependField != -1) {
                    ...
                    // Convert the server data to a Java object
                    if (f instanceof HexFieldDescription) { // Field is a hex field, setDataType to indicate correct length
                    ...
                    } else { // character field - setDataType to indicate correct length
                ...
                // Check for possible variable length field
                    ...
                    } else { // Field is not variable length
                        fields_[i] = dType.toObject(contents, offset);
                        recordLength_ += dType.getByteLength();
                    }
                } else { // Not a VariableLengthFieldDescription
                    fields_[i] = dType.toObject(contents, offset);
                    recordLength_ += dType.getByteLength();
                }
            ...
            if (recordDescriptionListeners_ != null) fireFieldModifiedEvent();
        } else { // No dependent fields; we can convert on the fly as necessary
    ...
    public void setField(int index, Object value) {
        ...
        if (!hasDependentFields_) { // Keep the contents of as400Data_ up to date so that
...
//@RBA
    ...
    public String toString() {
        ...
        Object obj = null;
        for (int i = 0; i < fields_.length; ++i) { // Append each field as a String to theRecord.  Separate each field's
