...
public class UserSpace implements Serializable {
    ...
    public UserSpace(AS400 system, String path) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing UserSpace object, system: " + system + " path: " + path);
        // Validate arguments.
        if (system == null) {
    ...
    private synchronized void chooseImpl() throws AS400SecurityException, IOException {
        ...
        if (impl_ == null) {
            ...
            if (mustUseSockets_)
                impl_ = (UserSpaceImpl) system_.loadImpl2("com.ibm.as400.access.UserSpaceImplRemote", "com.ibm.as400.access.UserSpaceImplProxy");
            else
                impl_ = (UserSpaceImpl) system_.loadImpl3("com.ibm.as400.access.UserSpaceImplNative", "com.ibm.as400.access.UserSpaceImplRemote", "com.ibm.as400.access.UserSpaceImplProxy");
    ...
    public boolean isMustUseProgramCall() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if user space must use program call:", mustUseProgramCall_);
    ...
    public boolean isMustUseNativeMethods() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if user space must use native methods:", mustUseNativeMethods_);
