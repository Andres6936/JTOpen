...
public class AS400JDBCConnectionImpl
    ...
    public void cancel(int id)
        ...
        synchronized (cancelLock_) {
            if (TESTING_THREAD_SAFETY)
                return; // in certain testing modes, don't contact the system
            cancelling_ = true;
    ...
    public void checkOpen()
            throws SQLException {
        if (TESTING_THREAD_SAFETY)
            return; // in certain testing modes, don't contact IBM i system
        if ((server_ == null))
...
//@G4M Moved code to createStatement (int, int, int)
    ...
    public void setConcurrentAccessResolution(int concurrentAccessResolution) throws SQLException {
        ...
        try {
            ...
            if (reply != null) {
                reply.returnToPool();
                reply = null; // Can return -- only errorClass accessed
            }
        }
    ...
    public boolean isClosed()
            throws SQLException {
        if (aborted_ && (Thread.currentThread() != abortingThread_))
            return true;  /*@D7A*/

        if (TESTING_THREAD_SAFETY)
            return false; // in certain testing modes, don't contact IBM i system
    ...
    */
     ...
     * @throws SQLException If the SQL statement has a syntax error.
     **/
     ...
     * @return The prepared statement object.
     * @throws SQLException - If connecting to IBM i V5R4 or earlier systems,
     ...
     * @throws SQLException if a database access error occurs or the given Savepoint
...
//@P1D            requestPending_[id] = true; //@P0A
        ...
        if (vrm_ >= JDUtilities.vrm530) {
            ...
            try {
                ...
                if (reply != null) {
                    reply.returnToPool();
                    reply = null;
                } // Return to pool since only errorClass accessed
            }
        }
    }
    ...
    public void setProperties(JDDataSourceURL dataSourceUrl, JDProperties properties, AS400Impl as400, boolean newServer, boolean skipSignonServer)
        ...
        if (maximumBlockedInputRows_ > 32000)
            maximumBlockedInputRows_ = 32000;                     // @A6A
        if (maximumBlockedInputRows_ < 1)
            maximumBlockedInputRows_ = 1;                             // @A6A
        ...
        if (numericRangeError == null)
            numericRangeError = JDProperties.NUMERIC_RANGE_ERROR_DEFAULT;
    ...
    public void setServerAttributes()
        ...
        try {
            ...
            try {
                ...
                // Server CCSID:               277
                // Default SQL Library:        XL�KODTA              -- E7 D3 80 D2 D6 C4 E3 C1
                ...
                */
                ...
                if (properties_.getBoolean(JDProperties.DATA_COMPRESSION)) {            // @ECA
                    if (vrm_ >= JDUtilities.vrm510) {                           // @ECA
                ...
                if (vrm_ >= JDUtilities.vrm440) {                  // @D0C @E9C
        ...
        catch (UnsupportedEncodingException e) {                      // @J5C
            JDError.throwSQLException(this, JDError.EXC_INTERNAL, e);
        } finally {
...
//            JDError.throwSQLException( this, JDError.EXC_INTERNAL, e);
            ...
            if (setClientInfoReply != null) {
                ...
                setClientInfoReply = null; // only error class used
            }
        }
    }
