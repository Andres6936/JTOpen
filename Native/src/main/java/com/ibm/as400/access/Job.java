...
public class Job implements Serializable {
    ...
    public Job(AS400 system, String jobName, String userName, String jobNumber) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing Job object, system: " + system + ", job name: " + jobName + ", user name: " + userName + ", job number: " + jobNumber);
        if (system == null) {
    ...
    public Job(AS400 system, String internalJobID) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing Job object, system: " + system + ", job identifier: '" + internalJobID + "'");
        if (system == null) {
            Trace.log(Trace.ERROR, "Parameter 'system' is null.");
            throw new NullPointerException("system");
        }
    ...
    Job(AS400 system, String name, String user, String number, String status, String type, String subtype) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing Job object, system: " + system + ", job name: " + name + ", user name: " + user + ", job number: " + number + ", status: " + status + ", type: " + type + ", subtype: " + subtype);
        system_ = system;
        name_ = name;
    ...
    /**
     * Adds a PropertyChangeListener.  The specified PropertyChangeListener's {@link PropertyChangeListener#propertyChange propertyChange()} method will be called each time the value of any bound property is changed.
     *
    ...
    /**
     * Adds a VetoableChangeListener.  The specified VetoableChangeListener's {@link VetoableChangeListener#vetoableChange vetoableChange()} method will be called each time the value of any constrained property is changed.
     *
     * @param listener The listener.
    ...
    private final ProgramCall getProgramCall(String pgm, ProgramParameter[] parms, Boolean callOnThread) {
        if (ON_THREAD.equals(callOnThread)) {
            ...
            try {
                pgmCall_onThread_.setProgram(pgm, parms);
            } catch (PropertyVetoException e) {
            }
            ...
            try {
                pgmCall_offThread_.setProgram(pgm, parms);
            } catch (PropertyVetoException e) {
            }
    ...
    public CallStackEntry[] getCallStack(long threadID) throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException {
        ...
        if (numEntriesReturned != numEntriesAll) {
            if (Trace.traceOn_)
                Trace.log(Trace.WARNING, "Not all call stack entries were returned: total: " + numEntriesAll + ", returned: " + numEntriesReturned);
        }
        threadID = BinaryConverter.byteArrayToLong(data, 20);
        if (data[28] != 0x40) {
            if (Trace.traceOn_)
                Trace.log(Trace.WARNING, "Call stack entry information incomplete due to status: " + Integer.toHexString(0x00FF & data[28]));
        }
     ...
     * </ul>
     * @throws AS400SecurityException          If a security or authority error occurs.
     * @throws ErrorCompletingRequestException If an error occurs before the request is completed, or if the job can no longer be found (AS400Exception)..
     * @throws InterruptedException            If this thread is interrupted.
     * @throws IOException                     If an error occurs while communicating with the system.
     * @throws ObjectDoesNotExistException     If the object does not exist on the system.
     * @see #COMPLETION_STATUS
    ...
    public String getStringValue(int attribute) throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException {
        ...
        if (obj == null) return null;
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Returning a string representation of the value of a non-String attribute:", attribute);
            return obj.toString();
        }
    }
