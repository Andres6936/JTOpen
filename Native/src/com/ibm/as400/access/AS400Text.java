...
public class AS400Text implements AS400DataType {
    ...
    private void setTable() {
        if (tableImpl_ == null) {
            if (Trace.traceOn_)
                Trace.log(Trace.CONVERSION, "AS400Text object initializing, encoding: " + encoding_ + ", CCSID: " + ccsid_ + ", system: " + system_);
            if (encoding_ != null) {
                try {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.CONVERSION, "AS400Text object initialized, encoding: " + encoding_ + ", CCSID: " + ccsid_ + ", system: " + system_ + ", table: " + tableImpl_);
        }
    }
...
//        }
    ...
    public int toBytes(Object javaValue, byte[] serverValue, int offset, BidiConversionProperties properties) {
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.CONVERSION, "AS400Text.toBytes(): Converted javaValue (" + toConvert + ") to:", serverValue, offset, length_);
