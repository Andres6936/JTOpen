...
class DDMDataStream extends DataStream {
    ...
    void write(OutputStream out) throws IOException {
        ...
        if (Trace.isTraceOn())
            Trace.log(Trace.DATASTREAM, "DDMDataStream.write() (connID=" + connectionID_ + "):", data_, 0, bytesToWrite);
        ...
        if (data_.length > bytesToWrite) {
            ...
            for (int i = bytesToWrite, packetLength = 0; i < data_.length; i += packetLength - 2) {
                ...
                if (packetLength > MAX_DATA_STREAM_LEN) {
                    ...
                    if (Trace.isTraceOn())
                        Trace.log(Trace.DATASTREAM, "DDMDataStream.write() continuation (connID=" + connectionID_ + "):", new byte[]{(byte) 0xFF, (byte) 0xFF});
                } else {
                    ...
                    if (Trace.isTraceOn())
                        Trace.log(Trace.DATASTREAM, "DDMDataStream.write() packetLength (connID=" + connectionID_ + "):", new byte[]{(byte) (packetLength >> 8), (byte) packetLength});
                }
