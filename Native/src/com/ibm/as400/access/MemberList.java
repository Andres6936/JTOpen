...
public class MemberList {
    ...
    public MemberList(AS400 system, QSYSObjectPathName path) {
        ...
        if (memberName == null || memberName.length() == 0) {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Setting member selection to *ALL.  Member name from QSYSObjectPathName object is: " + memberName);
        } else {
            memberSelection_ = memberName;
        }
    }
    ...
    private Date transformDate(String dateString) {
        ...
        if (dateString == null || dateString.length() == 0) {
            ...
            try {
                ...
                if (Trace.traceOn_)
                    Trace.log(Trace.ERROR, "Ignored error while parsing date string: " + dateString, pe);
            }
        }
