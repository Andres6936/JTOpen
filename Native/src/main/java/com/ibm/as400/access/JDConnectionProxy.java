...
// JDBC 3.0
   ...
   * The possible values for this property are {@link AS400JDBCDataSource#CONCURRENTACCESS_NOT_SET},
   * {@link AS400JDBCDataSource#CONCURRENTACCESS_USE_CURRENTLY_COMMITTED},
   * {@link AS400JDBCDataSource#CONCURRENTACCESS_WAIT_FOR_OUTCOME} and
   * {@link AS400JDBCDataSource#CONCURRENTACCESS_SKIP_LOCKS},
   * with the property defaulting to {@link AS400JDBCDataSource#CONCURRENTACCESS_NOT_SET}.
   * Setting this property to default exhibits the default behavior on the servers  
   * i.e., the semantic applied for read 
   * transactions to avoid locks will be determined by the server.          
   *
   * {@link AS400JDBCDataSource#CONCURRENTACCESS_USE_CURRENTLY_COMMITTED} specifies that driver will flow USE CURRENTLY COMMITTED
   * to server.  Whether CURRENTLY COMMITTED will actually be in effect is
   * ultimately determined by server. 
   *
   * {@link AS400JDBCDataSource#CONCURRENTACCESS_WAIT_FOR_OUTCOME} specifies that driver will flow WAIT FOR OUTCOME
   * to server.  This will disable the CURRENTLY COMMITTED behavior at the server,
   * if enabled, and the server will wait for the commit or rollback of data in the process of
   * being updated.  
   *   
   * {@link AS400JDBCDataSource#CONCURRENTACCESS_SKIP_LOCKS} specifies that driver will flow SKIP LOCKS
   * to server.  This directs the database manager to skip records in the case of record lock conflicts. 
   * 
   *  @param concurrentAccessResolution The current access resolution setting.  Possible valuse:
   *  {@link AS400JDBCDataSource#CONCURRENTACCESS_NOT_SET},
   *  {@link AS400JDBCDataSource#CONCURRENTACCESS_USE_CURRENTLY_COMMITTED},
   *  {@link AS400JDBCDataSource#CONCURRENTACCESS_WAIT_FOR_OUTCOME}, or
   *  {@link AS400JDBCDataSource#CONCURRENTACCESS_SKIP_LOCKS}
   * @throws SQLException  If a database error occurs.
   */
