...
public class SaveFile
    ...
    private static String getObjectType(Product product)
        ...
        if (type == null || type.length() == 0) {
            type = "*ALL";
        } else if (type.equals(Product.LOAD_TYPE_CODE)) {
            type = "*PGM";
        } else if (type.equals(Product.LOAD_TYPE_LANGUAGE)) {
            type = "*LNG";
        } else {
    ...
    public void setMaximumNumberOfRecords(long maximumNumberOfRecords)
        ...
        String value;
        if (maximumNumberOfRecords == NO_MAX) {
            value = "*NOMAX";
        } else {
            value = Long.toString(maximumNumberOfRecords);
        }
...
//  Threadsafe: No
        ...
        if (entryCCSID == 0)
            entryCCSID = systemCCSID; // From the API spec: "The coded character set ID for data in the list entries.  If 0, then the data is not associated with a specific CCSID and should be treated as hexadecimal data."
