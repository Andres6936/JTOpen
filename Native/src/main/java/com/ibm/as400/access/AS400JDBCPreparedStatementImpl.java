...
public class AS400JDBCPreparedStatementImpl extends AS400JDBCPreparedStatement {
     ...
     * @throws SQLException If the SQL statement contains a syntax error or an error
     *                      occurs.
     **/
     ...
     *
     * @throws SQLException If the statement is not open or an input parameter has not
     *                      been set.
     **/
     ...
     *
     * @param sql The SQL statement to be added to the current batch. This can be
     *            any SQL statement that does not return a result set.
     * @throws SQLException This exception is always thrown.
     **/
            ...
            if (((parameterCount_ > 0) && (descriptorHandle_ == 0))
                ...
                for (int i = 0; i < parameterCount_; ++i) {
                    if (!parameterRow_.containsArray_ || parameterRow_.isInput(i + 1)) // @array4
                    {
                        ...
                        if (sqlData.getType() == Types.ARRAY) // @array
                        {
                        ...
                        for (int rowLoop = 0; rowLoop < rowCount; ++rowLoop) // @G9a
                        {
                            ...
                            for (int i = 0; i < parameterCount_; ++i) {
                                ...
                                // //@EIC
                                {
                                    ...
                                    if (sqlData.getType() != Types.ARRAY)
                                        parameterMarkerData
     ...
     * @throws SQLException If the statement is not open, the query timeout limit is
     ...
     * @throws SQLException If the statement is not open, an SQL statement contains a
     ...
     * @throws SQLException If the statement is not open, no result set is returned by the
     ...
     *
     * @param parameterIndex The parameter index (1-based).
     * @param parameterValue The parameter value or null to set the value to SQL NULL.
     * @param length         The number of bytes in the stream.
     * @throws SQLException If the statement is not open, the index is not valid, the
     ...
     * @param parameterIndex The parameter index (1-based).
     * @param parameterValue The parameter value or null to set the value to SQL NULL.
     * @throws SQLException If the statement is not open, the index is not valid, or the
     ...
     * @param parameterIndex The parameter index (1-based).
     * @param sqlType        The SQL type code defined in java.sql.Types.
     * @param typeName       The fully-qualified name of an SQL structured type. This value
     *                       will be ignored.
     * @throws SQLException If the statement is not open, the index is not valid, the
     *                      parameter is not an input parameter, or the SQL type is not
     *                      valid.
     ...
     * @param parameterValue The parameter value or null to set the value to SQL NULL.
     * @param sqlType        The SQL type code defined in java.sql.Types.
     * @param scale          The number of digits after the decimal if sqlType is DECIMAL or
     *                       NUMERIC.
     * @throws SQLException If the statement is not open, the index is not valid, the
     *                      parameter is not an input parameter, the SQL type is not
...
/* ifdef JDBC40
     ...
     *
     * @param parameterIndex The parameter index (1-based).
     * @param parameterValue The parameter value or null if the value is SQL NULL.
     * @param calendar       The calendar, or null if not applicable.
...
/* ifdef JDBC40
     ...
     * @throws SQLException If the SQL type is not compatible.
     **/
...
/* ifdef JDBC40
  ...
  /* ifdef JDBC40
     ...
     *
     * @param parameterIndex The parameter index (1-based).
     * @param reader         the <code>java.io.Reader</code> object that contains the Unicode
     *                       data
