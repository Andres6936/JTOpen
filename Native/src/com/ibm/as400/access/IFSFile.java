 ...
 * As in {@link File java.io.File}, IFSFile is designed to work
        ...
        implements Serializable, Comparable            // @B9c
{
     ...
     * @throws ConnectionDroppedException If the connection is dropped unexpectedly.
     * @throws ExtendedIOException        If an error occurs while communicating with the system.
     * @throws InterruptedIOException     If this thread is interrupted.
     * @throws ServerStartupException     If the host server cannot be started.
     * @throws UnknownHostException       If the system cannot be located.
     **/
...
// @B9a
     ...
     * @throws ServerStartupException     If the host server cannot be started.
     * @throws UnknownHostException       If the system cannot be located.
     * @throws IOException                If the user is not authorized to create the file.
     **/
...
// @D4A
     ...
     * @throws IOException                If an error occurs while communicating with the system.
     * @throws ConnectionDroppedException If the connection is dropped unexpectedly.
     * @throws ExtendedIOException        If an error occurs while communicating with the system, or the directory is not accessible.
     * @throws InterruptedIOException     If this thread is interrupted.
     * @throws ServerStartupException     If the host server cannot be started.
     * @throws UnknownHostException       If the system cannot be located.
     **/
     ...
     * @throws AS400SecurityException If a security or authority error occurs.
     * @throws IOException            If an error occurs while communicating with the system.
     * @throws ExtendedIOException    If the file does not exist.
     **/
...
//@AC7 Start
    ...
    public String[] list(IFSFileFilter filter)
            throws IOException { // @A5D Removed null filter check (null means no filtering)
        return list(filter, "*");
    }
    ...
    String[] list0(IFSFileFilter filter, String pattern)
            throws IOException, AS400SecurityException { // @A5D removed check for null filter (null means no filtering).
    ...
    private IFSFile[] listFiles0(IFSFileFilter filter, String pattern, int maxGetCount, String restartName,
        ...
        if (Trace.isTraceOn())
            Trace.log(Trace.DIAGNOSTIC, "IFSFile::listFile0(): returned (" + listFiles0LastNumObjsReturned_ + ") pre objects");
        if (fileAttributeList == null) {
            ...
            if (Trace.isTraceOn())
                Trace.log(Trace.DIAGNOSTIC, "IFSFile::listFile0(): returned (" + files.length + ") post objects");
            return files;
        }
    }
     ...
     * @throws IOException                If an error occurs while communicating with the system.
     * @throws ConnectionDroppedException If the connection is dropped unexpectedly.
     * @throws ExtendedIOException        If an error occurs while communicating with the system, or if the file doesn't exist or is a directory.
     * @throws InterruptedIOException     If this thread is interrupted.
     * @throws ServerStartupException     If the host server cannot be started.
     * @throws UnknownHostException       If the system cannot be located.
     **/
