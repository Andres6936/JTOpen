...
public class UserIndex {
     ...
     * @param value               The value portion of the new entry
     * @param insertionRuleOption The rule used for inserting the entries. Valid values are :
    ...
    public boolean isMustUseNativeMethods() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if user index must use native methods:", mustUseNativeMethods_);
    ...
    public void setMustUseNativeMethods(boolean useNativeMethods) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Setting if user index must use native methods:", useNativeMethods);
