    ...
    private synchronized void refreshHelpText(PanelGroup pg) throws AS400Exception, AS400SecurityException,
        ...
        if (pGroup != null) {
            if (prdLib != null) {
                ...
                if (!exists) {
                    if (Trace.traceOn_)
                        Trace.log(Trace.DIAGNOSTIC, "Command.refreshHelpText: Adding " + prdLib + " to library list.");
            ...
            if (added) {
                if (Trace.traceOn_)
                    Trace.log(Trace.DIAGNOSTIC, "CommandHelpRetriever: Removing " + prdLib + " from library list.");
...
//  }
    ...
    private synchronized void refreshHelpIDs() throws AS400Exception, AS400SecurityException,
        ...
        if (prdLib != null) {
            ...
            if (!exists) {
                if (Trace.traceOn_)
                    Trace.log(Trace.DIAGNOSTIC, "Command.refreshHelpIDs: Adding " + prdLib + " to library list.");
                // We have to try to add it.
                String addlible = "ADDLIBLE LIB(" + prdLib + ")";  // not threadsafe
                CommandCall cc = new CommandCall(system_, addlible);
                added = cc.run();
...
//    initializeTransient();
    ...
    private synchronized void refreshCommandInfo() throws AS400Exception, AS400SecurityException,
        ...
        if (!succeeded) {
            ...
            {
                if (Trace.traceOn_)
                    Trace.log(Trace.WARNING, "PTF SI29629 is not installed: (MCH0802) " + msgList[msgList.length - 1].getText());
        ...
        try {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.WARNING, "Unable to process source file: '" + srcName + "," + srcLib + "," + srcMbr + "'.", eiae);
            sourceFile_ = null;
        }
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Successfully refreshed command information for " + path_ + ".");
    }
    ...
    private synchronized void refreshXML(boolean f) throws AS400Exception, AS400SecurityException,
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Refreshing XML (format " + format + ") information for " + path_ + ".");
        if (system_ == null)
            throw new ExtendedIllegalStateException("system", ExtendedIllegalStateException.PROPERTY_NOT_SET);
        if (path_ == null)
            throw new ExtendedIllegalStateException("path", ExtendedIllegalStateException.PROPERTY_NOT_SET);
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Refreshing XML /QSYS.LIB/QCDRCMDD.PGM returned bytesAvailable=" + bytesAvailable);
        ...
        if (f) {
            ...
            refreshedXML_ = true;
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Refreshing XML /QSYS.LIB/QCDRCMDD.PGM returned data=" + xml_);

        } else {
            xml2_ = conv1208.byteArrayToString(outputData, 8, bytesReturned, 0);
            refreshedXML2_ = true;
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Refreshing XML /QSYS.LIB/QCDRCMDD.PGM returned data=" + xml2_);
        }
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Successfully refreshed XML (format " + format + ") information for " + path_ + ".");
    }
