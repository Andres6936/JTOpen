...
public class AS400 implements Serializable {
    ...
    public AS400(String systemName, String userId) {
        ...
        if (isTurkish()) {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "This system locale is Turkish, userId.toUpperCase(Locale.ENGLISH)");
        }
        //@W9 End
    ...
    private AS400(String systemName, ProfileTokenProvider tokenProvider, Integer refreshThreshold) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing AS400 object with a profile token provider, system name: '" + systemName + "'");
    ...
    private AS400(String systemName, String userId, CredentialVault pwVault) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing internal AS400 object, system name: '" + systemName + "' user ID: '" + userId + "'");
    ...
    public static void addPasswordCacheEntry(String systemName, String userId, String password) throws AS400SecurityException, IOException {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Adding password cache entry, system name: '" + systemName + "' user ID: '" + userId + "'");
    ...
    public static void addPasswordCacheEntry(String systemName, String userId, String password, String proxyServer) throws AS400SecurityException, IOException {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Adding password cache entry, system name: '" + systemName + "' user ID: '" + userId + "' proxy server: '" + proxyServer + "'");
    ...
    private static int getNativeVersion() {
        try {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Not using native optimizations; class 'NativeVersion' is not found.");
            AS400.nativeVersion = 0;
        } catch (Exception e) {
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Not using native optimizations; unexpected exception while loading native version:", e);
            AS400.nativeVersion = 0;
        }
    ...
    public static int getPasswordExpirationWarningDays() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Getting the password expiration warning days:", AS400.expirationWarning);
    ...
    public static void removePasswordCacheEntry(String systemName, String userId) {
        ...
        String longName = null;
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Removing password cache entry, system name: " + systemName + " user ID: " + userId);
        if (systemName == null) {
            throw new NullPointerException("systemName");
        }
        if (userId == null) {
...
//            }
        ...
        synchronized (AS400.systemList) {
            ...
            for (int i = AS400.systemList.size() - 1; i >= 0; i--) {
                ...
                if (systemName.equalsIgnoreCase((String) curPtr[0]) && userId.equals(curPtr[1])) {
                    ...
                    if (Trace.traceOn_)
                        Trace.log(Trace.DIAGNOSTIC, "Adding password cache entry, removing previous entry.");
                    AS400.systemList.removeElementAt(i);
                }
            }
            ...
            // Now add the new one.
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Adding password cache entry for " + systemName + ":" + userId + ".");
    ...
    public static boolean setDefaultUser(String systemName, String userId) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Setting the default user, system name: '" + systemName + "' user ID: '" + userId + "'");
        if (systemName == null) {
            throw new NullPointerException("systemName");
        }
        if (userId == null) {
    ...
    private static boolean userIdMatchesLocal(String userId, boolean mustUseSuppliedProfile) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Determining if specified userID (" + userId + ") matches current userID.");
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Determined that specified userID (" + userId + ") " + (result ? "matches" : "does not match") + " current userID.");
    ...
    public void connectService(int service, int overridePort) throws AS400SecurityException, IOException {
        ...
        if (credVault_ instanceof ProfileTokenVault) {
            if (Trace.traceOn_)
                Trace.log(Trace.INFORMATION, "Before service connected, block the thread of refreshing profile token credential");
        ...
        try {
            ...
            if (credVault_ instanceof ProfileTokenVault) {
                if (Trace.traceOn_)
                    Trace.log(Trace.INFORMATION, "After service connected, notify the thread of refreshing profile token credential");
     ...
     *                        <ul>
     *                        <li>{@link ProfileTokenCredential#TYPE_SINGLE_USE TYPE_SINGLE_USE}
     *                        <li>{@link ProfileTokenCredential#TYPE_MULTIPLE_USE_NON_RENEWABLE TYPE_MULTIPLE_USE_NON_RENEWABLE}
     *                        <li>{@link ProfileTokenCredential#TYPE_MULTIPLE_USE_RENEWABLE TYPE_MULTIPLE_USE_RENEWABLE}
     *                        </ul>
    ...
    public void setUseSystemPasswordExpirationWarningDays(boolean useSystem) {
        if (useSystem) {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Don't use system password expiration(QPWDEXPWRN) warning.");
    ...
    public int getSystemPasswordExpirationWarningDays() throws AS400SecurityException, IOException {
        if (useSystemExpirationWarning_) {
            ...
            if (signonInfo_.PWDexpirationWarning > 0) {
                if (Trace.traceOn_)
                    Trace.log(Trace.DIAGNOSTIC, "Use system password expiration(QPWDEXPWRN) warning: " + signonInfo_.PWDexpirationWarning);
    ...
    public boolean isInPasswordExpirationWarningDays()
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if within the password expiration warning days for the system. ");
    ...
    public String getSystemName() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Getting system name: " + systemName_ + ", is local:", systemNameLocal_);
     ...
     * @throws AS400SecurityException          If a security or authority error
     *                                         occurs.
     * @throws ErrorCompletingRequestException If an error occurs before
     *                                         the request is completed.
     * @throws InterruptedException            If this thread is interrupted.
     * @throws IOException                     If an error occurs while communicating with
     *                                         the system.
     * @throws ObjectDoesNotExistException     If the API used to retrieve the information does not exist on the system.
    ...
    Object loadImpl(String impl1, String impl2) {
        if (canUseNativeOptimizations()) {
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Load of native implementation '" + impl1 + "' failed, attempting to load remote implementation.");
        }
    ...
    public boolean isMustAddLanguageLibrary() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if must add language library:", mustAddLanguageLibrary_);
    ...
    public boolean isMustUseSuppliedProfile() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if must use supplied profile:", mustUseSuppliedProfile_);
    ...
    synchronized void signon(boolean keepConnection) throws AS400SecurityException, IOException {
        ...
        if (signonInfo_ == null) {
            ...
            try {
                ...
                } else {  // Tolerate the exception - we don't require GSS.
            ...
            if (!keepConnection) {
                if (Trace.traceOn_)
                    Trace.log(Trace.DIAGNOSTIC, "Disconnecting temporary connection for validating signon info.");
                impl_.disconnect(AS400.SIGNON);
            }
        }
    }
