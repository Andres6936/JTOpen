...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    *********************************************************************************  -->
    <xsl:template match="QcdCLCmd">
        ...
        <xsl:choose>
            ...
            <xsl:when test="Cmd/@PromptMsgID!=''">
                <xsl:text/>&amp;msg(<xsl:value-of select="Cmd/@PromptMsgID"/>). -
                <xsl:value-of select="$_HELP"/><xsl:text/>
            ...
            <xsl:when test="Cmd/@Prompt!=''">
                <xsl:text/><xsl:value-of select="Cmd/@Prompt"/> -
                <xsl:value-of select="$_HELP"/><xsl:text/>
                :p.<xsl:value-of select="substring-before($_INTRO_COMMAND_HELP,'&amp;amp;1')"/><xsl:text/>
            ...
            <xsl:otherwise>
                <xsl:text/><xsl:value-of select="Cmd/@CmdName"/> -
                <xsl:value-of select="$_HELP"/><xsl:text/>
                :p.<xsl:value-of select="substring-before($_INTRO_COMMAND_HELP,'&amp;amp;1')"/><xsl:text/>
    ...
    <xsl:template match="Cmd">
        ...
        .*
        .*
        <xsl:value-of select="substring-before($_EXAMPLES_HEADING,'&amp;amp;1')"/>
        <xsl:value-of select="$CommandName"/>
        <xsl:value-of select="substring-after($_EXAMPLES_HEADING,'&amp;amp;1')"/>
        <xsl:text/>
        .*
        .**************************************************<xsl:text>&#xa;</xsl:text>
        ...
        <xsl:value-of select="substring-after($_EXAMPLES_HEADING,'&amp;amp;1')"/> -
        <xsl:value-of select="$_HELP"/><xsl:text/>
        :xh3.<xsl:value-of select="substring-before($_EXAMPLES_HEADING,'&amp;amp;1')"/>
        <xsl:value-of select="$CommandName"/>
        <xsl:value-of select="substring-after($_EXAMPLES_HEADING,'&amp;amp;1')"/>
        <xsl:text/>
        ...
        .*
        .*
        <xsl:value-of select="substring-before($_ERROR_MESSAGES_HEADING,'&amp;amp;1')"/>
        <xsl:value-of select="$CommandName"/>
        <xsl:value-of select="substring-after($_ERROR_MESSAGES_HEADING,'&amp;amp;1')"/>
        <xsl:text/>
        .*
        .**************************************************<xsl:text>&#xa;</xsl:text>
    ...
    <xsl:template match="Parm">
        ...
        <xsl:if test="@PosNbr!=''">
            ...
            <xsl:text/>.*
            <xsl:value-of select="$_DESCRIBE_PARAMETER"/><xsl:text/>
            ...
            <xsl:if test="@Max>1">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:text>&#xa;</xsl:text>
                        <xsl:text/>:p.<xsl:value-of
                            select="substring-before($_MULTIPLE_PARAMETER_VALUES_ALLOWED,'&amp;amp;1')"/>
                        <xsl:value-of select="@Max"/>
            ...
            <xsl:if test="@Dft!=''">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <xsl:text>&#xa;</xsl:text>
                        <xsl:text/>.*
                        <xsl:value-of select="$_DESCRIBE_PARAMETER_DEFAULT"/><xsl:text/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:if>
            ...
            <xsl:choose>
                ...
                <xsl:when test="@RangeMinVal!=''">
                    <xsl:text>&#xa;</xsl:text>:pt.:pv.<xsl:value-of select="@RangeMinVal"/>-<xsl:value-of
                        select="@RangeMaxVal"/><xsl:text/>:epv.<xsl:text/>
                    <xsl:text>&#xa;</xsl:text>:pd.<xsl:text>&#xa;</xsl:text>
                    <xsl:text/>&lt;...&gt;
                    <xsl:text>&#xa;</xsl:text>
                    <xsl:text/>.*
                    <xsl:value-of select="$_DESCRIBE_PARAMETER_VALUE_WITH_RANGE"/><xsl:text/>
                </xsl:when>
    ...
    <xsl:template match="Elem">
        ...
        <xsl:if test="not(@Constant) and @Type!='ZEROELEM'">
            ...
            <xsl:text/>:p.:hp2.<xsl:value-of select="$_ELEMENT"/><xsl:text> </xsl:text>
            <xsl:number count="Elem"/>:
            <xsl:text/>
            <xsl:choose>
                <xsl:when test="@PromptMsgID!=''">
            ...
            <xsl:if test="@Max>1">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:text>&#xa;</xsl:text>
                        <xsl:text/>:p.<xsl:value-of
                            select="substring-before($_MULTIPLE_ELEMENT_VALUES_ALLOWED,'&amp;amp;1')"/>
                        <xsl:value-of select="@Max"/>
    ...
    <xsl:template match="Qual">
        ...
        <xsl:if test="not(@Constant)">
            ...
            <xsl:text/>:p.:hp2.<xsl:value-of select="$_QUALIFIER"/><xsl:text> </xsl:text>
            <xsl:number count="Qual"/>:
            <xsl:text/>
            <xsl:choose>
    ...
    <xsl:template name="GenType">
        <xsl:choose>
            ...
            <xsl:when test="@Type='SNAME'">
                ...
                <xsl:text/>.*
                <xsl:value-of select="$_DESCRIBE_USERDEFINED_PARAMETER_VALUE"/><xsl:text/>
            </xsl:when>
    ...
    <xsl:template match="Value">
        ...
        <xsl:text>&#xa;</xsl:text>
        <xsl:text/>.*
        <xsl:value-of select="$_DESCRIBE_PREDEFINED_PARAMETER_VALUE"/><xsl:text/>
    </xsl:template>
