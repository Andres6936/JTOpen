...
class IFSWriteReq extends IFSDataStreamReq {
    ...
    private final void setOffsetFields(long fileOffset, int datastreamLevel) {
        if (datastreamLevel < 16) { // Just set the old fields.
            if (fileOffset > (long) Integer.MAX_VALUE) {
                if (Trace.traceOn_)
                    Trace.log(Trace.WARNING, "Specified fileOffset value (" + fileOffset + ") exceeds maximum file length supported by system.");
