...
extends ToolboxWrapper
     ...
     *            a result set.
     * @throws SQLException If the statement is not open or
        ...
        synchronized (connection_) {       /* lock the connection to prevent deadlock with connection.rollback */
            synchronized (internalLock_) {    // @E6A
                ...
                if (isAssociatedWithLocators() && (connection_.getProperties().getBoolean(JDProperties.HOLD_STATEMENTS)) && (connection_.getAutoCommit() == false)) {
                    ...
                    if (commonExecuteReply != null) {
                        commonExecuteReply.returnToPool();
                        commonExecuteReply = null;
                    }
                    ...
                    if (getMoreResultsReply != null) {
                        getMoreResultsReply.returnToPool();
                        getMoreResultsReply = null;
                    }
                ...
                if (rpbCreated_ && !connectionReset_) {    // @EDA
    ...
    */
     ...
     * @throws SQLException If the query timeout limit is
                ...
                if ((!allowImmediate_) || (!sqlStatement.isImmediatelyExecutable())) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (fetchFirstBlock)
                                resultData = commonExecuteReply.getResultData();
            ...
            if (nameOverride_.length() == 0) {
                ...
                //
                    ...
                    try {
                        ...
                        if (connection_.getVRM() >= JDUtilities.vrm540)
                            extended = true;                   //@540
     ...
     * DELETE, or 0 for SQL statements that
     * return nothing.
     * @throws SQLException If connection to IBM i V5R4 or earlier systems,
...
*/
     ...
     * @throws SQLException if DatabaseMetaData.supportsMultipleOpenResults returns false
