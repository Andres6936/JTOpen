...
public abstract class OpenList implements Serializable {
     ...
     * @throws IOException                     If an error occurs while communicating with the system.
     * @throws ObjectDoesNotExistException     If the object does not exist on the system.
     * @throws OpenListException               If the system is unable to correctly generate the list of items.
     * @see #open
     **/
    ...
    public synchronized Object[] getItems(int listOffset, int number) throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException, OpenListException {
        if (Trace.isTraceOn())
            Trace.log(Trace.DIAGNOSTIC, "Retrieving open list, list offset: " + listOffset + ", number:", number);
        if (listOffset < -1) {
        ...
        do {
            ...
            if (recordsReturned < number) {
                if (Trace.isTraceOn())
                    Trace.log(Trace.DIAGNOSTIC, "Retrieved messages, records returned: " + recordsReturned + ", number:", number);
        ...
        if (listStatusIndicator != 0xF2) {
            if (Trace.isTraceOn())
                Trace.log(Trace.ERROR, "Unable to build object list on server, list status indicator:", listStatusIndicator);
    ...
    public synchronized void open() throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException, OpenListException {
        ...
        if (Trace.isTraceOn())
            Trace.log(Trace.DIAGNOSTIC, "Opened object list, length: " + length_ + ", record length: " + BinaryConverter.byteArrayToInt(listInformation, 12) + ", handle:", handle_);
    }
