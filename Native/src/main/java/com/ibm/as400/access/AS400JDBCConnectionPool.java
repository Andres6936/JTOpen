...
public class AS400JDBCConnectionPool extends ConnectionPool implements Serializable {
     ...
     *
     * @throws SQLException If a database error occurs closing a connection.
     **/
     ...
     * @throws ConnectionPoolException          If a database error occurs creating a connection for the pool, or the maximum number of connections has been reached for the pool.
     * @throws ExtendedIllegalArgumentException if the number of connections to fill the pool with is less than one.
     **/
    public void fill(int numberOfConnections) throws ConnectionPoolException {
        ...
        try {
            synchronized (availablePool_) {
                for (int i = 0; i < numberOfConnections; i++) { //@A5A
...
//@CRS      else if (!maintenance_.isRunning())
            if (!maintenance_.isRunning())
                maintenance_.setRunning(true);            // Restart. @CRS
     ...
     * @throws ConnectionPoolException If a database error occurs getting the connection.
     **/
     ...
     * @throws ClassNotFoundException If a file error occurs.
     **/
