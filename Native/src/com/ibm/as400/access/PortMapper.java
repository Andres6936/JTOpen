...
class PortMapper {
    ...
    static SocketContainer getServerSocket(String systemName,
        ...
        if (overridePort > 0) {
            ...
            if (srvPort == AS400.USE_PORT_MAPPER) {
                ...
                if (Trace.traceOn_)
                    Trace.log(Trace.DIAGNOSTIC, "Adding entry to Service Port table: system " + systemName + ", service " + fullServiceName + ", port " + srvPort);
                PortMapper.setServicePort(systemName, service, srvPort, useSSL);
            }

        }
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Opening socket for service " + service + " to system..." + systemName + " port:" + srvPort);
    ...
    static Socket getSocketConnection(String systemName, int port, SocketProperties socketProperties) throws IOException {
        ...
        /*  Due to various jvm and compile issues, there are many possible types of exceptions that could
         ...
         solution to finding the jvm version that does not degrade performance. */
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Connect to socket: system '" + systemName + "', port " + port + ", login timeout " + loginTimeout + " ms.");
            ...
            while (!done)  // up to two tries
            {
                try {
                    ...
                    Method method = thisClass.getMethod("connect", new Class[]{SocketAddress.class, Integer.TYPE});
            ...
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Connect to port mapper: system '" + systemName + "', port " + port + ", no login timeout (JVM 1.3 or lower).");
    ...
    static void setSocketProperties(Socket socket, SocketProperties socketProperties) throws SocketException {
        ...
        if (socketProperties.receiveBufferSizeSet_) {
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Setting receive buffer size:", socketProperties.receiveBufferSize_);
            socket.setReceiveBufferSize(socketProperties.receiveBufferSize_);
        }

        if (socketProperties.sendBufferSizeSet_) {
            if (Trace.traceOn_)
                Trace.log(Trace.DIAGNOSTIC, "Setting send buffer size:", socketProperties.sendBufferSize_);
