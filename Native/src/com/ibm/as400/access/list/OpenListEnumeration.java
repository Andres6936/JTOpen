    ...
    public synchronized Object nextElement() {
        ...
        if (objectCache_ == null || cachePosition_ >= objectCache_.length) {
            try {
                ...
                if (Trace.isTraceOn())
                    Trace.log(Trace.DIAGNOSTIC, "Loaded next block in OpenListEnumeration: " + objectCache_.length + " messages at offset " + listOffset_ + " out of " + length_ + " total, using block size " + blockSize + ".");
            } catch (Exception e) {
                if (Trace.isTraceOn())
