...
public class MessageQueue implements Serializable {
    ...
    public MessageQueue(AS400 system, String path) {
        super();
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Constructing MessageQueue object, system: " + system + " path: " + path);
        if (system == null) {
            Trace.log(Trace.ERROR, "Parameter 'system' is null.");
            throw new NullPointerException("system");
        }
     ...
     *
     * @return An Enumeration of {@link QueuedMessage QueuedMessage} objects.
     * @throws AS400SecurityException          If a security or authority error occurs.
     * @throws ErrorCompletingRequestException If an error occurs before the request is completed.
     * @throws InterruptedException            If this thread is interrupted.
     * @throws IOException                     If an error occurs while communicating with the system.
     ...
     * @return The array of retrieved {@link QueuedMessage QueuedMessage} objects.  The length of this array may not necessarily be equal to <i>number</i>, depending upon the size of the list on the system, and the specified <i>listOffset</i>.
     * @throws AS400SecurityException          If a security or authority error occurs.
     * @throws ErrorCompletingRequestException If an error occurs before the request is completed.
     * @throws InterruptedException            If this thread is interrupted.
     * @throws IOException                     If an error occurs while communicating with the system.
    ...
    public QueuedMessage[] getMessages(int listOffset, int number) throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Retrieving message queue message list, list offset: " + listOffset + ", number:", number);
        if (listOffset < -1) {
    ...
    public byte[] getWorkstationStartingMessageKey() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Getting workstation starting message key:", workstationStartingMessageKey_);
    ...
    public void setWorkstationStartingMessageKey(byte[] workstationStartingMessageKey) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Setting workstation starting message key:", workstationStartingMessageKey);
        synchronized (this) {
    ...
    public boolean isSelectMessagesNeedReply() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if messages that need a reply are selected:", selectMessagesNeedReply_);
    ...
    public void setSelectMessagesNeedReply(boolean selectMessagesNeedReply) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Setting messages that need a reply are selected:", selectMessagesNeedReply);
        synchronized (this) {
    ...
    public boolean isSelectMessagesNoNeedReply() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if messages that do not need a reply are selected:", selectMessagesNoNeedReply_);
    ...
    public void setSelectMessagesNoNeedReply(boolean selectMessagesNoNeedReply) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Setting messages that do not need a reply are selected:", selectMessagesNoNeedReply);
        synchronized (this) {
    ...
    public boolean isSelectSendersCopyMessagesNeedReply() {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Checking if sender's copy messages that need a reply are selected:", selectSendersCopyMessagesNeedReply_);
    ...
    public void setSelectSendersCopyMessagesNeedReply(boolean selectSendersCopyMessagesNeedReply) {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Setting sender's copy messages that need a reply are selected:", selectSendersCopyMessagesNeedReply);
        synchronized (this) {
    ...
    public synchronized void load() throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException {
        ...
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Loaded message queue message list, length: " + length_ + ", data length: " + dataLength_ + ", handle: ", handle_);
    }
    ...
    public QueuedMessage receive(byte[] messageKey, int waitTime, String messageAction, String messageType) throws AS400SecurityException, ErrorCompletingRequestException, InterruptedException, IOException, ObjectDoesNotExistException {
        if (Trace.traceOn_)
            Trace.log(Trace.DIAGNOSTIC, "Receiving from message queue, waitTime: " + waitTime + ", messageAction: " + messageAction + ", messageType: " + messageType + ", message key:", messageKey);
        ...
        if (Trace.traceOn_ && bytesReturned < bytesAvailable)
            Trace.log(Trace.WARNING, "Possible truncation receiving from message queue, bytes returned: " + bytesReturned + ", bytesAvailable:", bytesAvailable);
