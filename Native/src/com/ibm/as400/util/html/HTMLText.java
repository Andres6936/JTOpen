...
public class HTMLText extends HTMLTagAttributes implements HTMLConstants, Serializable      // @Z1C
{
    ...
    public void setUnderscore(boolean underscore) throws PropertyVetoException {
        ...
        if (vetos_ != null)
            vetos_.fireVetoableChange("underscore", new Boolean(oldUnderscore), new Boolean(underscore)); //@CRS

        underscore_ = underscore;

        if (changes_ != null)
            changes_.firePropertyChange("underscore", new Boolean(oldUnderscore), new Boolean(underscore)); //@CRS
    }
