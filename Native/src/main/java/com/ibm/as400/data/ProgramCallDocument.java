 ...
 * </dl>
 */
public class ProgramCallDocument implements Serializable, Cloneable {                                                                   // @C1C @C3C
    /**
     ...
     * @throws PcmlException when the specified PCML document cannot be found
     */
     ...
     * @throws PcmlException        If an error occurs while processing XPCML.
    ...
    private static final void warnNull(String parmName) {
        if (Trace.isTraceOn())
            Trace.log(Trace.WARNING, "Null value specified for '" + parmName + "' parameter on ProgramCallDocument constructor.");
    }
     ...
     * The default bidi string type is assumed ({@link BidiStringType#DEFAULT BidiStringType.DEFAULT}).
     *
     * @param name The name of the &lt;data&gt; element in the PCML document.
    ...
    public Object getValue(String name, int[] indices)
            throws PcmlException {
        if (Trace.isTraceOn())
            Trace.log(Trace.PCML, "Entered method ProgramCallDocument.getValue(" + name + ", indices)");

        Object val = m_pcmlDoc.getValue(name, new PcmlDimensions(indices));

        if (Trace.isTraceOn())
            Trace.log(Trace.PCML, "Exiting method ProgramCallDocument.getValue(" + name + ", indices)");
     ...
     * @throws PcmlException If an error occurs while processing PCML.
     **/
     ...
     * @throws XmlException If an error occurs while processing RFML.
     **/
     ...
     * @throws IOException  If an error occurs while writing the data.
     * @throws XmlException If an error occurs while processing XPCML.
     **/
