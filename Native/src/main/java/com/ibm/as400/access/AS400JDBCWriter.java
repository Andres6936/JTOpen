...
class AS400JDBCWriter extends Writer {
    ...
    */
    public void write(char[] cbuf, int off, int len) throws IOException {
        if (cbuf == null) throw new NullPointerException("cbuf");
        if ((off < 0) || (off > len))
            throw new ExtendedIllegalArgumentException("off", ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
        if (len < 0)
            throw new ExtendedIllegalArgumentException("len", ExtendedIllegalArgumentException.PARAMETER_VALUE_NOT_VALID);
    ...
    */
    public synchronized void write(String str, int off, int len) throws IOException {
        ...
        try {
            ...
            else if (locator_ != null)
                locator_.setString(position_, str, off, len); //@PDC jdbc40
            else if (xml_ != null)
                xml_.setString(position_, str, off, len); //@PDA jdbc40
            position_ += len;
